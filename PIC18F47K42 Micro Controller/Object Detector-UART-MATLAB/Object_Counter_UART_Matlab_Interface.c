...
//
// Description:
//   This project implements a light-based object counter using an LDR sensor,
//   with access protected by a 2-digit password entered via a 4x3 keypad.
//
//   After entering the correct password, the system begins real-time monitoring
//   of ambient light levels via ADC and calculates an approximate lux value.
//   Each time the lux value drops below a threshold (indicating an object),
//   a counter is incremented. The current object count and lux level are
//   displayed on a 16x2 LCD.
//
// Features:
//   - 2-digit password entry using a 4x3 keypad
//   - Lux value computation from LDR (connected to RA0/AN0)
//   - Object detection based on lux threshold
//   - Real-time LCD display of lux and object count
//   - UART output of lux readings (optional debug)
//   - Reset functionality via button on RB0
//
// Inputs:
//   - LDR sensor on RA0 (AN0)
//   - Keypad on RB1?RB3 (columns), RB4?RB7 (rows)
//   - Reset button on RB0
//
// Outputs:
//   - 16x2 LCD: Data lines RD4?RD7, RS on RC2, EN on RC3
//   - UART2: Optional for serial lux output
//
// Notes:
//   - Threshold for object detection is lux < 800
//   - Password is hardcoded as "23"
//------------------------------------------------------------------------------
