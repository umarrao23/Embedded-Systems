//------------------------------------------------------------------------------
// Title     : LDR-Based Light Monitoring System with Interrupt-Triggered Wait Mode
//------------------------------------------------------------------------------
// Purpose   : Continuously monitors ambient light using an LDR and displays the
//             light intensity (in lux) on an LCD. When an external interrupt
//             (e.g., a button press on RB1) is triggered, the system enters a 
//             "WAIT" mode, during which an LED blinks for 10 seconds.
//
// Features  :
//   - Analog-to-Digital Conversion (ADC) reads LDR voltage on RA0 (AN0)
//   - Lux value calculated and displayed in real time on a 16x2 LCD
//   - External interrupt on RB1 triggers a temporary "WAIT" state
//   - LED on RB0 blinks during WAIT mode for 10 seconds
//   - System resumes normal light monitoring after WAIT mode ends
//
...
// Outputs   : LCD on RD0?RD7 (EN on RC2, RS on RC3), LED on RB0
//------------------------------------------------------------------------------
