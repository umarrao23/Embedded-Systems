...
//Step 1) open generic user guide/ search systick/bit 1 is tickint/it enables systick interrupt/exception when set to 1


//search Cortex m4 Generic user Guide on goole download it/ Search SysTick/Page 249/
//Bit 0 is enable/disable / bit 2 is clock source// Bit 16 is count flag

//Create a function for systick delay MS
void systickDelayMs(int delay)
{
//Step2 Load value here
	//Reload Systick with number of cycles per millisecond.
	SysTick->LOAD = SysTick_LOAD_VAl-1;//-1 because we count from 0 // inside our structure which is loaded into .h file the reload registers is simply called load
...
//step 3 )Create a function for systick delay MS
void systick_1hz_interrupt(void)
{

	SysTick->LOAD = (ONE_SEC_LOAD-1); // inside our structure which is loaded into .h file the reload registers is simply called load
